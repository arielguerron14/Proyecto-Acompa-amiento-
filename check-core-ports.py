...
print("🔍 Checking Docker containers listening ports on EC2-Core...")
cmd = "docker ps --format 'table {{.Names}}\\t{{.Ports}}'"
stdin, stdout, stderr = ssh.exec_command(cmd)
print(stdout.read().decode())

print("\n🔍 Testing local connectivity to micro-auth...")
cmd = "curl -s http://localhost:3000/health 2>&1 || echo 'Connection failed'"
stdin, stdout, stderr = ssh.exec_command(cmd)
