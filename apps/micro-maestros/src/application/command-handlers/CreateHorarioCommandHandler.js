...
class CreateHorarioCommandHandler {
  ...
  async handle(command) {
    try {
      // Check for overlaps
      const hasOverlap = await this.horarioRepository.checkOverlap(command.maestroId, command.dia, command.inicio, command.fin);
      if (hasOverlap) {
...
module.exports = CreateHorarioCommandHandler;
