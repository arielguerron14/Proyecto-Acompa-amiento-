#!/usr/bin/env python3
import paramiko
import io

SSH_KEY_CONTENT = """-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----"""

HOST = "3.236.51.29"
USER = "ubuntu"

# Updated .env files content with correct MongoDB connection via service name
env_files = {
    "micro-auth/.env": """NODE_ENV=production
SERVICE_NAME=micro-auth
MONGO_URL=mongodb://mongo:27017/acompaamiento
PORT=3000
LOG_LEVEL=info
""",
    "micro-estudiantes/.env": """NODE_ENV=production
SERVICE_NAME=micro-estudiantes
MONGO_URL=mongodb://mongo:27017/acompaamiento
PORT=3001
LOG_LEVEL=info
""",
    "micro-maestros/.env": """NODE_ENV=production
SERVICE_NAME=micro-maestros
MONGO_URL=mongodb://mongo:27017/acompaamiento
PORT=3002
LOG_LEVEL=info
""",
    "micro-reportes-estudiantes/.env": """NODE_ENV=production
SERVICE_NAME=micro-reportes-estudiantes
POSTGRES_URL=postgresql://admin:MyPostgresProd123!@postgres:5432/acompaamiento
MONGO_URL=mongodb://mongo:27017/acompaamiento
PORT=5003
LOG_LEVEL=info
""",
    "micro-reportes-maestros/.env": """NODE_ENV=production
SERVICE_NAME=micro-reportes-maestros
POSTGRES_URL=postgresql://admin:MyPostgresProd123!@postgres:5432/acompaamiento
MONGO_URL=mongodb://mongo:27017/acompaamiento
PORT=5004
LOG_LEVEL=info
""",
}

ssh = paramiko.SSHClient()
ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
key = paramiko.RSAKey.from_private_key(io.StringIO(SSH_KEY_CONTENT))
ssh.connect(HOST, username=USER, pkey=key, timeout=30)

print("📝 Updating .env files with correct MongoDB URLs...")
for file_path, content in env_files.items():
    cmd = f'cat > ~/Proyecto-Acompa-amiento-/{file_path} << \'EOF\'\n{content}EOF'
    stdin, stdout, stderr = ssh.exec_command(cmd)
    print(f"✅ Updated {file_path}")

print("\n🔄 Restarting docker-compose.core.yml...")
cmd = """
cd ~/Proyecto-Acompa-amiento- && \
sudo docker-compose -f docker-compose.core.yml down && \
sleep 3 && \
sudo docker-compose -f docker-compose.core.yml up -d && \
sleep 15 && \
docker ps --format 'table {{.Names}}\\t{{.Status}}'
"""
stdin, stdout, stderr = ssh.exec_command(cmd)
print(stdout.read().decode())

ssh.close()
print("✅ Services updated and restarted!")
