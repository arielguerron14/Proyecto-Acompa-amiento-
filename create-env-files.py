#!/usr/bin/env python3
import paramiko
import io

SSH_KEY_CONTENT = """-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----"""

HOST = "3.236.51.29"
USER = "ubuntu"

# Create .env files content
env_files = {
    "micro-auth/.env": """NODE_ENV=production
SERVICE_NAME=micro-auth
MONGO_URL=mongodb://admin:MyMongoProd123!@localhost:27017/acompaamiento?authSource=admin
PORT=3000
LOG_LEVEL=info
""",
    "micro-estudiantes/.env": """NODE_ENV=production
SERVICE_NAME=micro-estudiantes
MONGO_URL=mongodb://admin:MyMongoProd123!@localhost:27017/acompaamiento?authSource=admin
PORT=3001
LOG_LEVEL=info
""",
    "micro-maestros/.env": """NODE_ENV=production
SERVICE_NAME=micro-maestros
MONGO_URL=mongodb://admin:MyMongoProd123!@localhost:27017/acompaamiento?authSource=admin
PORT=3002
LOG_LEVEL=info
""",
    "micro-reportes-estudiantes/.env": """NODE_ENV=production
SERVICE_NAME=micro-reportes-estudiantes
POSTGRES_URL=postgresql://admin:MyPostgresProd123!@localhost:5432/acompaamiento
MONGO_URL=mongodb://admin:MyMongoProd123!@localhost:27017/acompaamiento?authSource=admin
PORT=5003
LOG_LEVEL=info
""",
    "micro-reportes-maestros/.env": """NODE_ENV=production
SERVICE_NAME=micro-reportes-maestros
POSTGRES_URL=postgresql://admin:MyPostgresProd123!@localhost:5432/acompaamiento
MONGO_URL=mongodb://admin:MyMongoProd123!@localhost:27017/acompaamiento?authSource=admin
PORT=5004
LOG_LEVEL=info
""",
    "api-gateway/.env": """NODE_ENV=production
SERVICE_NAME=api-gateway
PORT=8080
AUTH_SERVICE_URL=http://localhost:3000
ESTUDIANTES_SERVICE_URL=http://localhost:3001
MAESTROS_SERVICE_URL=http://localhost:3002
REPORTES_EST_SERVICE=http://localhost:5003
REPORTES_MAEST_SERVICE=http://localhost:5004
LOG_LEVEL=info
CORS_ORIGIN=http://44.220.126.89,http://52.7.168.4
"""
}

ssh = paramiko.SSHClient()
ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
key = paramiko.RSAKey.from_private_key(io.StringIO(SSH_KEY_CONTENT))
ssh.connect(HOST, username=USER, pkey=key, timeout=30)

print("📝 Creating .env files in EC2-Core...")
for file_path, content in env_files.items():
    cmd = f'cat > ~/Proyecto-Acompa-amiento-/{file_path} << \'EOF\'\n{content}EOF'
    stdin, stdout, stderr = ssh.exec_command(cmd)
    print(f"✅ Created {file_path}")

ssh.close()
print("✅ All .env files created successfully!")
