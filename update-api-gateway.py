#!/usr/bin/env python3
import paramiko
import io

SSH_KEY_CONTENT = """-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----"""

# Core microservices private IP
CORE_PRIVATE_IP = "172.31.79.241"
API_GATEWAY_HOST = "52.7.168.4"
USER = "ubuntu"

# Updated .env content
env_content = f"""PORT=8080
AUTH_SERVICE_URL=http://{CORE_PRIVATE_IP}:3000
ESTUDIANTES_SERVICE_URL=http://{CORE_PRIVATE_IP}:3001
MAESTROS_SERVICE_URL=http://{CORE_PRIVATE_IP}:3002
REPORTES_EST_SERVICE=http://{CORE_PRIVATE_IP}:5003
REPORTES_MAEST_SERVICE=http://{CORE_PRIVATE_IP}:5004
LOG_LEVEL=info
CORS_ORIGIN=http://44.220.126.89
"""

ssh = paramiko.SSHClient()
ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
key = paramiko.RSAKey.from_private_key(io.StringIO(SSH_KEY_CONTENT))
ssh.connect(API_GATEWAY_HOST, username=USER, pkey=key, timeout=30)

print("📝 Updating API Gateway .env configuration...")
cmd = f'cat > ~/Proyecto-Acompa-amiento-/api-gateway/.env << \'EOF\'\n{env_content}EOF'
stdin, stdout, stderr = ssh.exec_command(cmd)
print("✅ Updated .env file")

print("\n🔄 Restarting API Gateway container...")
cmd = """
cd ~/Proyecto-Acompa-amiento- && \
sudo docker-compose -f docker-compose.api-gateway.yml restart api-gateway && \
sleep 5 && \
curl -s http://localhost:8080/health || echo "Service not ready yet"
"""
stdin, stdout, stderr = ssh.exec_command(cmd)
print(stdout.read().decode())

ssh.close()
print("✅ API Gateway configuration updated and restarted!")
